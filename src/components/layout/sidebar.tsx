...
export function Sidebar({ open = false, onClose }: SidebarProps) {
  ...
  const handleToggleOnline = async () => {
    ...
    try {
      ...
      console.error("[Sidebar] failed to toggle online status:", err);
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left transition-colors hover:bg-slate-800/60 focus:bg-slate-800/60 focus:outline-none data-popup-open:bg-slate-800/60">
              <div className="relative shrink-0">
                ...
                <span
                  className={`absolute -bottom-0.5 -right-0.5 block h-2.5 w-2.5 rounded-full ring-2 ring-slate-900 transition-colors duration-200 ${
            ...
            >
              <div className="px-2 py-1.5 flex items-center justify-between">
                <div className="flex items-center gap-1.5 min-w-0">
                  ...
                  <span className="text-xs font-medium text-slate-300">Availability</span>
                </div>
                <button
                  type="button"
